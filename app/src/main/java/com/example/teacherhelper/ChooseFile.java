...
public class ChooseFile {
    ...
    private void upListAdapter() {
        ...
        File directory = new File(pathString.get(l - 1));//存储目录信息
//        System.out.println(pathString.get(l-1));
//        String fileList[] = directory.list();
//
//        for (int i = 0; i < fileList.length; i++) { // 遍历返回的字符数组
//            System.out.println(fileList[i]);
//        }
//        File f = new File("/storage/emulated/0/Documents/studentExport.xls");
//        if (f.exists()) System.out.println(1111);
//        System.out.println(files.length);
        File[] files = directory.listFiles();//文件列表
        if (files != null) {
...
//        L.e("getLoacalBitmap:::" + url);
        Log.v("ChooseFile","getLoacalBitmap:::" + url);
        try {
